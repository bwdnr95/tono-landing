"""
Vercel Serverless Function — TONO Landing API
모든 /api/* 요청을 처리하는 단일 엔트리포인트
"""
...
async def send_slack_notification(data: ContactRequest) -> None:
    ...
    payload = {
        ...
        "blocks": [
            {"type": "header", "text": {"type": "plain_text", "text": "새 상담 신청이 접수되었습니다!", "emoji": True}},
...
SHEET_HEADERS = ["접수일시", "이름", "연락처", "이메일", "지역", "숙소 유형", "객실 수", "관심 서비스", "메시지"]
...
async def append_to_sheet(data: ContactRequest) -> None:
    ...
    row = [
        ...
        data.location, data.propertyType, data.rooms,
      ...
      <table width="580" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:14px;overflow:hidden;box-shadow:0 2px 16px rgba(0,0,0,0.06);">
        ...
        <tr>
          <td style="padding:44px 40px 36px;">
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="background:#faf8f5;border:1px solid #e8e4de;border-radius:10px;overflow:hidden;">
              <tr><td style="padding:18px 22px 6px;"><span style="font-size:10px;color:#b8845f;letter-spacing:2px;text-transform:uppercase;font-weight:600;">신청자 정보</span></td></tr>
              <tr><td style="padding:6px 22px 18px;">
                <table width="100%" cellpadding="0" cellspacing="0">
                  <tr><td style="padding:9px 0;font-size:13px;color:#9e978e;width:90px;border-bottom:1px solid #e8e4de;">이름</td><td style="padding:9px 0;font-size:13px;color:#1a1815;font-weight:600;border-bottom:1px solid #e8e4de;">{data.name}</td></tr>
                  <tr><td style="padding:9px 0;font-size:13px;color:#9e978e;width:90px;border-bottom:1px solid #e8e4de;">연락처</td><td style="padding:9px 0;font-size:13px;color:#1a1815;border-bottom:1px solid #e8e4de;">{data.phone}</td></tr>
                  <tr><td style="padding:9px 0;font-size:13px;color:#9e978e;width:90px;">이메일</td><td style="padding:9px 0;font-size:13px;color:#1a1815;">{data.email}</td></tr>
                </table>
              </td></tr>
            </table>
            <table width="100%" cellpadding="0" cellspacing="0" style="background:#faf8f5;border:1px solid #e8e4de;border-radius:10px;overflow:hidden;margin-top:16px;">
              <tr><td style="padding:18px 22px 6px;"><span style="font-size:10px;color:#b8845f;letter-spacing:2px;text-transform:uppercase;font-weight:600;">신청 내역</span></td></tr>
              <tr><td style="padding:6px 22px 18px;">
                <table width="100%" cellpadding="0" cellspacing="0">
                  <tr><td style="padding:9px 0;font-size:13px;color:#9e978e;width:90px;border-bottom:1px solid #e8e4de;">숙소 위치</td><td style="padding:9px 0;font-size:13px;color:#1a1815;border-bottom:1px solid #e8e4de;">{data.location or '-'}</td></tr>
                  <tr><td style="padding:9px 0;font-size:13px;color:#9e978e;width:90px;border-bottom:1px solid #e8e4de;">숙소 유형</td><td style="padding:9px 0;font-size:13px;color:#1a1815;border-bottom:1px solid #e8e4de;">{data.propertyType or '-'}</td></tr>
                  <tr><td style="padding:9px 0;font-size:13px;color:#9e978e;width:90px;border-bottom:1px solid #e8e4de;">객실 수</td><td style="padding:9px 0;font-size:13px;color:#1a1815;border-bottom:1px solid #e8e4de;">{data.rooms or '-'}</td></tr>
                  <tr><td style="padding:9px 0;font-size:13px;color:#9e978e;width:90px;border-bottom:1px solid #e8e4de;">관심 서비스</td><td style="padding:9px 0;font-size:13px;color:#1a1815;border-bottom:1px solid #e8e4de;">{data.service or '-'}</td></tr>
                  <tr><td style="padding:9px 0;font-size:13px;color:#9e978e;width:90px;vertical-align:top;">문의 내용</td><td style="padding:9px 0;font-size:13px;color:#6b6560;line-height:1.6;">{data.message or '-'}</td></tr>
                </table>
              </td></tr>
            </table>
            <table width="100%" cellpadding="0" cellspacing="0" style="margin-top:28px;">
              <tr><td style="padding:18px 0;border-top:1px solid #e8e4de;">
                <p style="margin:0;font-size:12px;color:#9e978e;line-height:1.8;">문의사항이 있으시면 언제든 연락해 주세요.</p>
                <table cellpadding="0" cellspacing="0" style="margin-top:10px;">
                  <tr><td style="padding:3px 0;font-size:12px;color:#6b6560;"><span style="color:#b8845f;margin-right:6px;">&#9742;</span> 064-763-9500 <span style="color:#d8d3cb;margin:0 8px;">|</span> 09:00 ~ 21:00 연중무휴</td></tr>
                  <tr><td style="padding:3px 0;font-size:12px;color:#6b6560;"><span style="color:#b8845f;margin-right:6px;">&#9993;</span> contact@tono-operation.com</td></tr>
                </table>
              </td></tr>
            </table>
          </td>
        ...
        <tr>
          <td style="padding:22px 40px;background:#faf8f5;text-align:center;border-top:1px solid #e8e4de;">
            <span style="font-size:10px;color:#9e978e;letter-spacing:1px;">Beyond Operation, Tone the Space.</span><br>
            <span style="font-size:10px;color:#d8d3cb;margin-top:4px;display:inline-block;">&copy; TONO OPERATION. All rights reserved.</span>
          </td>
        </tr>
      </table>
    </td></tr>
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#0a0a0a;padding:32px 0;">
    <tr><td align="center">
      <table width="560" cellpadding="0" cellspacing="0">
        <tr><td style="padding:24px 32px;border-bottom:1px solid #1e1c18;"><span style="font-size:10px;color:#b8845f;letter-spacing:3px;text-transform:uppercase;font-weight:700;">NEW CONSULTATION</span></td></tr>
